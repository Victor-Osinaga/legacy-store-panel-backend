...
export default async function verifySubdomain(req, res, next) {
  try {
    ...
    const matchdev = normalizedOrigin.match(allowedOriginPatternFrontStore);
    ...
    if (matchdev) {
      ...
      const getClientAdminBySubdomain =
        await clientAdminService.getClientAdminBySubdomain(subdomain);
      console.log(
        "getClientAdminBySubdomain : middleware : verify subdomain",
        getClientAdminBySubdomain
      );
