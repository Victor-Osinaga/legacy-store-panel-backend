...
export default class StoreConfigurationMongo {
  ...
  async createStoreConfiguration(storeConfigDto) {
    try {
      ...
      if (!savedStoreConfig)
        throw {
          msg: "Error en BD al crear la store configuration",
          status: 500,
        };
      const newStoreConfig = await this.collection
        .findOne({ id: storeConfigDto.id }, { _id: 0, __v: 0 })
        .lean();
  ...
  async getStoreConfigurationByName(storeConfigName) {
    try {
      const configByName = await this.collection
        .findOne({ storeConfigName: storeConfigName }, { _id: 0, __v: 0 })
        .lean();
  ...
  async getStoreConfiguration() {
    try {
      const configStore = await this.collection
        .findOne({}, { _id: 0, __v: 0 })
        .lean();
  ...
  async getStoreConfigurationById(configId) {
    return await this.collection
      .findOne({ id: configId }, { _id: 0, __v: 0 })
      .lean();
  }
  ...
  async updateStoreConfiguration(idConfig, newData) {
    ...
    return await this.collection
      .findOne({ id: idConfig }, { _id: 0, __v: 0 })
      .lean();
  }

  async updateLogoStoreConfig(id, newUrlLogo) {
    try {
      const updatedLogo = await this.collection.findOneAndUpdate(
        { id: id },
        { $set: { "logoConfig.logoUrl": newUrlLogo } },
        { new: true }
      );

      return updatedLogo;
    } catch (error) {
      console.log("error desde container updateLogoStoreConfig", error);
      throw error;
    }
  }
