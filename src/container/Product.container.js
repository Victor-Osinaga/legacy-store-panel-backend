...
export default class ProductMongo {
  ...
  async connect(url, db) {
    ...
    if (this.connections[connectionIdentifier]) {
      ...
      if (existingConnection.readyState === 1) {
        console.log(
          "Reutilizando la conexión existente para:",
          connectionIdentifier
        );
        this.collection = existingConnection.model(
          this.collectionName,
          this.schema
        );
  ...
  async updateCategoriesProductsAll(categoryId, newCategories) {
    try {
      const result = await this.collection.updateMany(
        { "categories.0.categoria.id": categoryId }, // Filtro
        { $set: { categories: newCategories } } // Actualización
      );

      console.log(`${result.modifiedCount} productos actualizados.`);
      return result;
    } catch (error) {
      console.log("desde container : updateCategoriesProductsAll", error);
      throw error;
    }
  }
  ...
  async createProduct(productDto) {
    try {
      ...
      if (!savedProduct)
        throw { msg: "Error en BD al crear el producto", status: 500 };
      const newProduct = await this.collection
        .findOne({ id: productDto.id }, { _id: 0, __v: 0 })
        .lean();
  ...
  async getProductStoreById(id) {
    try {
      console.log(
        "getProductStoreById",
        await this.collection.findOne({ id: id }, { _id: 0, __v: 0 }).lean()
      );
      return await this.collection
        .findOne({ id: id }, { _id: 0, __v: 0 })
        .lean();
    } catch (error) {
