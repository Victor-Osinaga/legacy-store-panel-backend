...
export default class ShipmentDeliveryMongo {
  ...
  async createShipmentDelivery(data) {
    try {
      ...
      if (!savedShipmentDelivery)
        throw { msg: "Error en BD al crear SHIPMENT DELIVERY", status: 599 };

      const newShipmentDelivery = await this.collection
        .findOne({ id: data.id }, { _id: 0, __v: 0 })
        .lean();
      return newShipmentDelivery;
    } catch (error) {
      console.log(
        "error desde ShipmentDeliveryMongo : createShipmentDelivery",
        error
      );
      throw error;
    }
  ...
  async updateShipmentDeliveryById(idShipingDelivery, data) {
    ...
    return await this.collection
      .findOne({ id: idShipingDelivery }, { _id: 0, __v: 0 })
      .lean();
  }
  ...
  async getShipmentsDeliveryStore() {
    try {
      const shipmentsDeliveryStore = await this.collection
        .find({}, { _id: 0, __v: 0 })
        .lean();
