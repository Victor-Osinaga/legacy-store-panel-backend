...
export default class CategoryMongo {
  ...
  async getUncategorized(uncategorizedName) {
    const uncat = await this.collection
      .findOne({ name: uncategorizedName }, { _id: 0, __v: 0 })
      .lean();
  ...
  async deleteAllByParentNodeIdAndId(parentNodeId) {
    return await this.collection.deleteMany({
      $or: [{ parentNodeId: parentNodeId }, { id: parentNodeId }],
    });
  }

  async getAllByParentNodeIdAndId(parentNodeId) {
    return await this.collection
      .find({
        $or: [{ parentNodeId: parentNodeId }, { id: parentNodeId }],
      })
  ...
  async updatePrimaryById(id, data) {
    try {
      const updatedCategory = await this.collection
        .findOneAndUpdate(
          { id: id }, // Buscar por UUID en lugar de ObjectId
          { name: data },
      ...
      console.log("error desde container updatePrimaryById: ", error);
  ...
  async createCategory(categoryDto) {
    try {
      ...
      if (!savedCategory)
        throw { msg: "Error en BD al crear la categoria", status: 500 };
      return await this.collection
        .findOne({ id: savedCategory.id }, { _id: 0, __v: 0 })
        .lean();
    } catch (error) {
      throw error;
    }
