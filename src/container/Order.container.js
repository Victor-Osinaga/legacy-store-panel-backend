...
export default class OrderMongo {
  ...
  async createOrder(orderDto) {
    try {
      ...
      if (!savedOrder)
        throw { msg: "Error en BD al crear la ORDER", status: 500 };
      return await this.collection
        .findOne({ id: orderDto.id }, { _id: 0, __v: 0 })
        .lean();
    } catch (error) {
  ...
  async getOrderById(external_reference) {
    return await this.collection
      .findOne({ id: external_reference }, { _id: 0, __v: 0 })
      .lean();
  }

  async deleteOrderById(id) {
    const deletedOrder = await this.collection
      .deleteOne({ id: id }, { _id: 0, __v: 0 })
      .lean();
  ...
  async getOrderStatusById(orderId) {
    return await this.collection
      .findOne({ id: orderId }, { _id: 0, __v: 0 })
      .lean();
  }
}
