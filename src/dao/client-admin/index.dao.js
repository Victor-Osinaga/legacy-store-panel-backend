...
switch (config.env) {
  case "dev":
    ...
    clientAdminDao = new ClientAdminDevDAO(
      "clients",
      clientAdminSquema,
      config.dev_url_database_admin
    );
    break;

  default:
    console.log("modo prod");
    const { default: ClientAdminProdDAO } = await import(
      "./ClientAdminProd.dao.js"
    );
    clientAdminDao = new ClientAdminProdDAO(
      "clients",
      clientAdminSquema,
      config.prod_url_database_admin
    );
    break;
}
