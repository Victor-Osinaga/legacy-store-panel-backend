...
async function getShipmentLocalDao(dbName) {
  ...
  if (config.env == "dev") {
    const { default: ShipmentLocalDevDAO } = await import(
      "./ShipmentLocalDev.dao.js"
    );
    shipmentLocalDao = new ShipmentLocalDevDAO(
      "shipmentLocals",
      shipmentLocalSquema,
      `${config.dev_url}${dbName}`
    );
  } else {
    const { default: ShipmentLocalProdDAO } = await import(
      "./ShipmentLocalProd.dao.js"
    );
    shipmentLocalDao = new ShipmentLocalProdDAO(
      "shipmentLocals",
      shipmentLocalSquema,
