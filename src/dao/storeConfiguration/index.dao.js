...
async function getStoreConfigurationDao(dbName) {
  ...
  if (config.env == "dev") {
    const { default: StoreConfigurationDevDAO } = await import(
      "./StoreConfigurationDev.dao.js"
    );
    storeConfigurationDao = new StoreConfigurationDevDAO(
      "storeconfigurations",
      storeConfigurationSchema,
      `${config.dev_url}${dbName}`
    );
  } else {
    const { default: StoreConfigurationProdDAO } = await import(
      "./StoreConfigurationProd.dao.js"
    );
    storeConfigurationDao = new StoreConfigurationProdDAO(
      "storeconfigurations",
      storeConfigurationSchema,
...
export { getStoreConfigurationDao };
