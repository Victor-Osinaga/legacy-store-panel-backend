...
async function getShipmentDeliveryDao(dbname) {
  ...
  if (config.env == "dev") {
    const { default: ShipmentDeliveryDevDAO } = await import(
      "./ShipmentDeliveryDev.dao.js"
    );
    shipmentDeliveryDao = new ShipmentDeliveryDevDAO(
      "shipmentDeliverys",
      shipmentDeliverySchema,
      `${config.dev_url}${dbname}`
    );
  } else {
    const { default: ShipmentDeliveryProdDAO } = await import(
      "./ShipmentDeliveryProd.dao.js"
    );
    shipmentDeliveryDao = new ShipmentDeliveryProdDAO(
      "shipmentDeliverys",
      shipmentDeliverySchema,
