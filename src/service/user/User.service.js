...
class UserService {
  ...
  async createUser(body) {
    try {
      ...
      const userByEmail = await this.userRepository.repoGetUserByEmail(
        newUserNoDto.getEmail()
      );
      if (userByEmail != null)
        throw { msg: "Email ya registrado como admin", status: 400 };

      newUserNoDto.setPassword(
        await bcryptjs.hash(newUserNoDto.getPassword(), 8)
      );

      const registeredUserNoDto = await this.userRepository.repoCreateUser(
        newUserNoDto.convertToDTO()
      );
  ...
  async createUserAdmin(body) {
    try {
      ...
      const userByEmail = await this.userRepository.repoGetUserByEmail(
        newUserAdminNoDto.getEmail()
      );
      if (userByEmail != null)
        throw { msg: "Email ya registrado", status: 400 };

      newUserAdminNoDto.setPassword(
        await bcryptjs.hash(newUserAdminNoDto.getPassword(), 8)
      );

      const registeredUserNoDto = await this.userRepository.repoCreateUser(
        newUserAdminNoDto.convertToDTO()
      );
