...
class ClientAdminService {
  ...
  async createClientAdmin(body) {
    try {
      ...
      const clientAdminByEmail =
        await this.clientAdminRepository.repoGetClientAdminByEmail(
          newClientAdminDto.getEmail()
        );
      console.log("clientAdminByEmail", clientAdminByEmail);

      if (clientAdminByEmail != null)
        throw { msg: "Email ya registrado", status: 400 };

      const clientAdminByProyectName =
        await this.clientAdminRepository.repoGetClientAdminByProyectName(
          newClientAdminDto.getProyectName()
        );
      if (clientAdminByProyectName != null)
        throw { msg: "Nombre del proyecto ya registrado", status: 400 };

      const clientAdminBySubdomain =
        await this.clientAdminRepository.repoGetClientAdminBySubdomain(
          newClientAdminDto.getSubdomain()
        );
      if (clientAdminBySubdomain != null)
        throw { msg: "Subdominio ya registrado", status: 400 };

      newClientAdminDto.setPassword(
        await bcryptjs.hash(newClientAdminDto.getPassword(), 8)
      );

      const registeredClientAdmin =
  ...
  async loginClientAdmin(body) {
    try {
      ...
      const clientByEmail =
        await this.clientAdminRepository.repoGetClientAdminByEmail(body.email);
      if (!clientByEmail)
  ...
  async getClientAdminById(clientId) {
    try {
      const clientById =
        await this.clientAdminRepository.repoGetClientAdminById(clientId);
      if (!clientById) {
