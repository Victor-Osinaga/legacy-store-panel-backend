...
//     const configuration =
//       await storeConfigurationService.createStoreConfiguration();
...
const getStoreConfiguration = async (req, res) => {
  ...
  try {
    const storeConfigurationService = await storeConfigurationFactory(dbname);
    const configuration =
      await storeConfigurationService.getStoreConfiguration();
    res.status(200).json({ status: "ok", data: configuration });
  } catch (error) {
    console.log("error desde controller storeConfiguration", error);
...
const updateStoreConfiguration = async (req, res) => {
  ...
  try {
    ...
    const updatedConfigStore =
      await storeConfigurationService.updateStoreConfiguration(
        req.params.id,
        req.body
      );
...
const updateLogoStoreConfig = async (req, res) => {
  const dbname = req.proyectName;
  try {
    console.log("req.body: ", req.body);
    console.log("req.file: ", req.file);

    const storeConfigService = await storeConfigurationFactory(dbname);

    // TODO: CONTINUAR

    const updatedLogo = await storeConfigService.updateLogoStoreConfig(req);
    return res.status(200).json({ status: "ok", data: updatedLogo });
  } catch (error) {
    console.log("error desde controller updateLogoStoreConfig", error);
...
const getStoreConfigurationStore = async (req, res) => {
  ...
  try {
    const storeConfigurationService = await storeConfigurationFactory(dbname);
    const configuration =
      await storeConfigurationService.getStoreConfigurationStore();
    res.status(200).json({ status: "ok", data: configuration });
  } catch (error) {
    // console.log("error desde controller storeConfiguration", error);
